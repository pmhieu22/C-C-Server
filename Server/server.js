...
db = mysql.createPool({
  ...
  password: config_mysql_pswd,
  database: config_mysql_db,
});

// console.log("abc: ", config_mysql_host);
...
app.use(
  sessions({
    ...
    secret: "fg56h674fs86j7fg64hrst784dsjgg41jhftg4j8",
...
console.log("[SYSTEM]".blue + " Autheur : duyphan");
...
app.get(
  "/login",
  middlewares.dependency_loggedOut_app,
  middlewares.dependency_logs_app,
  app_login.loginget
);
app.get(
  "/logout",
...
app.get(
  "/home",
  middlewares.logged_app,
  middlewares.dependency_loggedIn_app,
  middlewares.dependency_logs_app,
  app_home.home
);
app.post(
...
app.post(
  ...
  middlewares.dependency_loggedIn_app,
  middlewares.dependency_logs_app,
  app_client.client_dashboard_post
);
app.get(
  "/client_delete/:id",
  middlewares.logged_app,
  middlewares.dependency_loggedIn_app,
  middlewares.dependency_logs_app,
  app_client.client_dashboard_delete
);
app.get(
  "/client_remote_shell/:id",
  middlewares.logged_app,
  middlewares.dependency_loggedIn_app,
  middlewares.dependency_logs_app,
  app_client.client_remote_shell
);
...
app.post(
  ...
  middlewares.dependency_loggedIn_app,
  middlewares.dependency_logs_app,
  app_config.home_post
);
// [user]
var app_user = require("./routes/app/user");
app.get(
  "/user",
  middlewares.logged_app,
  middlewares.dependency_loggedIn_app,
  middlewares.dependency_logs_app,
  app_user.user_listing
);
app.post(
  "/user",
  middlewares.logged_app,
...
webAdmin.on("connection", function (socket) {
  ...
  socket.on("initWebAdmin", function (data) {
    var adminCheck =
      "SELECT * FROM t_admins WHERE admin_mail = " +
      db.escape(data.admin_mail) +
      " AND admin_password = " +
      db.escape(data.admin_password) +
      " AND admin_active = 1;";
    db.query(adminCheck, function (err, rows, fields) {
      if (rows.length != 0) {
        ...
        webAdmin
          ...
          .emit(
            "liveLogClient",
            logs + socket.id + " / " + clientAddressIp,
            "success"
          );
        middlewareWebAdminAuthorized(socket, admin, clientAddressIp);
      } else {
...
function middlewareWebAdminAuthorized(socket, admin, clientAddressIp) {
...
function webAdminAuthorized(socket, admin, clientAddressIp) {
  // =================== LOGS ====================== \\
  var logs = "Authorized admin passed middleware & auth: ";
  console.log(
    logs.green,
    "webAdmin-" + admin.admin_id + " (" + admin.admin_pseudo + ")",
    clientAddressIp
  );
  webAdmin
    .to("webAdmin")
  ...
  socket.on("getNodeClientOnline", function () {
    ...
    onlineClientsIo.forEach(function (onlineClientIo) {
      var ClientOnline =
        "SELECT client_id FROM t_clients WHERE client_socket = " +
        db.escape(onlineClientIo) +
        " ";
      db.query(ClientOnline, function (err, rows, fields) {
        if (rows.length == 1)
          webAdmin
            .to("webAdmin-" + admin.admin_id)
            .emit("nodeClientOnline", rows[0].client_id);
      });
    });
  });
  ...
  socket.on("cmdForReverseShell", function (clientId, adminId, cmd) {
    ...
    // console.log(data);
    nodeClient.to("nodeClient-" + clientId).emit("cmdForReverseShell", data);
  });
  socket.on("downloaded", function (clientId, path, name) {
    var data = {
      client_Id: clientId,
      path: path,
      name: name,
    ...
    // console.log(data);
    nodeClient.to("nodeClient-" + clientId).emit("downloaded", data);
  });
  // ================ Future features... ================= \\
  //............
  //............
...
db.query(config, function (err, rows, fields) {
  var config = rows[0];
  // console.log(
  //   "[SYSTEM]".green + " Cloud mode authentification : " + config.config_mode
  // );
  ...
  nodeClient.on("connection", function (socket) {
    ...
    socket.on("initNodeClient", function (data) {
      ...
      if (config.config_mode == "close") {
        console.log("ok");
        var cloudCheckAuth =
          "SELECT * FROM t_configs WHERE config_password = " +
          db.escape(
            crypto
              .createHash("sha1")
              .update(
        ...
        db.query(cloudCheckAuth, function (err, rows, fields) {
          if (rows.length != 0) {
            var clusterCheckExist =
              "SELECT * FROM t_clusters WHERE client_serial = " +
              db.escape(data.client_serial) +
              " AND client_key = " +
              db.escape(data.client_key);
            db.query(clusterCheckExist, function (err, rows, fields) {
              if (rows.length != 0) {
                //find client by cluster_id
                var { id } = rows[0];
                console.log("id: ", id);
                console.log("data: ", data);
                var clientCheckExist =
                  "SELECT * FROM t_clients WHERE cluster_id = " +
                  db.escape(id) +
                  " AND mac_addr like " +
                  db.escape(data.mac_addr);
                console.log("query: ", clientCheckExist);
                db.query(clientCheckExist, function (err, rows, fields) {
                  if (rows.length != 0) {
                    var client = rows[0];
                    console.log(client);
                    ...
                    middleWareNodeClientAuthorized(
                      socket,
                      clientAddressIp,
                      client
                    );
                    ...
                    console.log(logs.red, socket.id, clientAddressIp);
                    var addNewClient = `INSERT INTO t_clients (cluster_id, mac_addr) VALUES (${id}, ${db.escape(
                      data.mac_addr
                    )})`;
                    console.log("query log: ", addNewClient);
                    db.query(addNewClient, function (err, rows, fields) {
                      if (err) {
                        console.log("Error: ", err);
                      } else {
                        var logs =
                          "This Client is registered2 to database: " +
                          db.escape(data.mac_addr);
                        console.log(logs.green);
                        var clientCheckExist2 =
                          "SELECT * FROM t_clients WHERE  mac_addr = " +
                          db.escape(data.mac_addr);
                        db.query(
                          clientCheckExist2,
    ...
    socket.on("disconnect", function (data) {
      ...
      var disconnect =
        "SELECT * FROM t_clients WHERE client_socket = " +
        db.escape(socket.id) +
        " ;";
...
function middleWareNodeClientAuthorized(socket, clientAddressIp, client) {
  db.query(
    "UPDATE t_clients SET client_socket = '" +
      socket.id +
      "' WHERE client_id = '" +
      client.client_id +
      "'"
...
function nodeClientAuthorized(socket, clientAddressIp, client) {
  console.log(
    "Authorized client passed middleware & auth: ".green,
    socket.id,
    clientAddressIp
  );
  webAdmin
    ...
    .emit(
      "liveLogClient",
      "Authorized client passed middleware & auth: " +
        socket.id +
        " / " +
        clientAddressIp,
      "success"
  ...
  // ================ Update Client Info ==================== \\
  nodeClient
    .to("nodeClient-" + client.client_id)
    .emit("clientInitSuccess", client);
  //change code
  socket.on("clientUpdateInfo", function (data) {
    ...
    db.query(
      "UPDATE t_clients " +
        "SET client_hostname = " +
        db.escape(data.hostname) +
        ", " +
        "client_ip = " +
        db.escape(clientAddressIp) +
        ", " +
        "client_os = " +
        db.escape(data.platform) +
        ", " +
        "client_os_version = " +
        db.escape(data.arch) +
        ", " +
        "cpu_info = " +
        db.escape(data.cpu_info) +
        ", " +
        "mac_addr = " +
        db.escape(data.mac_addr) +
        ", " +
        ...
        ", " +
        "network = " +
        db.escape(data.network) +
        ", " +
        "client_privilege_access = '0', " + // not yet coded...
        "client_country = '/' " + // not yet coded...
        "WHERE client_id = " +
        db.escape(client.client_id) +
        ""
    );
  });

  // =========== Reverse shell callback client =========== \\
  socket.on("reverseShellCallback", function (data) {
    webAdmin
      .to("webAdmin-" + data.adminId)
      .emit("reverseShellCallback", data.clientId, data.type, data.cmdOutPut);
  });
